// chat-admin.js — מרכז ההודעות של מנהל־העל.
//
// עד לפיצול הכל ישב ב-chat.js, ולכן כל משתמש מחובר שהמודול נטען אצלו
// (ולו רק כדי לצייר את תג ההודעות שלא נקראו) הוריד גם את ניהול הפניות,
// השליחה הקבוצתית וניהול התשובות. הקובץ הזה נטען רק בדף הניהול.
//
// מצב השיחה הפעילה נשאר בבעלות chat.js, ומכאן הוא משתנה דרך הסטר.

import {
    setActiveConversation,
    activeConversationIsAdmin,
    markActiveConversationRead,
    collectAdminMessageReplies,
    normalizeConversationSticker,
    conversationStickerById
} from './chat.js';

// נמעני השליחה הקבוצתית ומצב השליחה — מצב של מרכז ההודעות בלבד.
const adminMessageRecipients = new Set();
let adminMessageSending = false;

// מסנן רשימת הפניות. מצב של מרכז ההודעות בלבד, ולכן הוא חי כאן.
...
window.openAdminConversation = function(uid) {
    ...
    setActiveConversation('admin', profile.uid);
    ...
    markActiveConversationRead().catch(error => console.warn('Marking admin chat read failed:', error));
};

window.toggleConversationResolved = async function() {
    if (!window.checkSuperAdminPermission() || !activeConversationIsAdmin()) return;
    const profile = window.activeConversationProfile?.();
    if (!profile) return;
